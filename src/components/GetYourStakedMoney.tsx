...
const GetYourStakedMoney: React.FC<GetYourStakedMoneyProps> = ({
  contract,
  address,
  gasLimit,
  stakedMoney,
  ...
  async function getYourStakedMoney() {
    if (contract !== null && address !== "") {
      const { output } = await contract.query["getYourStakedMoney"](
        address,
        {
          gasLimit: gasLimit,
          storageDepositLimit,
  ...
  return (
    <>
      <div className={styles.container_number}>
        <button className={styles.rotatebutton} onClick={getYourStakedMoney}>
          Staked
        </button>
